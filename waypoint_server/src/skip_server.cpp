#include "waypoint_server/skip_server.hpp"

using namespace std::chrono_literals;

waypoint_server::SkipServer::SkipServer(const rclcpp::NodeOptions &options) : Node("skip_server", options)
{
    // Create Client For Server Apply
    auto client_cb_group_ = create_callback_group(rclcpp::CallbackGroupType::Reentrant);
    server_apply_ = create_client<waypoint_server_msgs::srv::ServerInfo>("server_apply", rmw_qos_profile_services_default, client_cb_group_);
...
void waypoint_server::SkipServer::Callback(const std::shared_ptr<waypoint_server_msgs::srv::ServerInfo::Request> request, std::shared_ptr<waypoint_server_msgs::srv::ServerInfo::Response> response)
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(waypoint_server::SkipServer)
